...
def classify_images(images_dir, results_dic, model):
    ...
    for img_file in results_dic:
        ...
        img_class = classifier(path, model)

   
        
    ## Populates empty dictionary with both labels &indicates if they match (idx 2)
        label_classified = img_class.lower().strip()

        results_dic[img_file].append(label_classified)
        ...
        if results_dic[img_file][0] in label_classified:
            results_dic[img_file].append(1)

         # if pet image label was NOT found then there is no match
   
        else:
            results_dic[img_file].append(0)
